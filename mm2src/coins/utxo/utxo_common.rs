...
pub async fn send_taker_spends_maker_payment<T: UtxoCommonOps + SwapOps>(
    ...
    let time_lock = try_tx_s!(args.time_lock.try_into());
    common::log::info!("send_taker_spends_maker_payment args.secret.len={} args.secret={} secret_hash.len()={} secret_hash={}", args.secret.len(), hex::encode(args.secret), args.secret_hash.len(), hex::encode(args.secret_hash));
    let redeem_script = payment_script(
        time_lock,
        args.secret_hash,
        &try_tx_s!(Public::from_slice(args.other_pubkey)),
