...
use crate::nft::nft_structs::{ContractType, ConvertChain, NftInfo, TransactionNftDetails, WithdrawErc1155,
                              WithdrawErc721};
use crate::{DexFee, EthGasLimitOption, RpcCommonOps, ValidateWatcherSpendInput, WatcherSpendType};
...
use common::executor::{AbortOnDropHandle, AbortSettings, SpawnAbortable};
...
use ethcore_transaction::{Action, TransactionWrapper, TransactionWrapperBuilder as UnSignedEthTxBuilder, TxType,
...
impl EthCoin {
    ...
    fn estimate_gas_for_contract_call(&self, contract_addr: Address, call_data: Bytes) -> Web3RpcFut<U256> {
        ...
        Box::new(
            coin.get_swap_pay_for_gas_option(fee_policy_for_estimate)
    ...
    fn get_trade_fee(&self) -> Box<dyn Future<Item = TradeFee, Error = String> + Send> {
        ...
        Box::new(
            self.get_swap_pay_for_gas_option(self.get_swap_transaction_fee_policy())
                ...
                .and_then(move |pay_for_gas_option| {
                    let fee = calc_total_fee(U256::from(ETH_MAX_TRADE_GAS), &pay_for_gas_option)
                        .map_err(|e| e.to_string())?;
...
fn signed_tx_from_web3_tx(transaction: Web3Transaction) -> Result<SignedEthTx, String> {
    ...
    let tx_type = if transaction.transaction_type.is_none() || transaction.transaction_type.unwrap() == type_0 {
        TxType::Legacy
    } else if transaction.transaction_type.unwrap() == type_1 {
        TxType::Type1
...
pub async fn eth_coin_from_conf_and_request(
    ...
    let platform_fee_estimator_ctx = try_s!(FeeEstimatorContext::new(ctx, conf, &coin_type).await);
