...
pub mod common_impl {
    ...
    use crate::utxo::{UtxoCommonOps, utxo_common::checked_address_from_str};
        ...
        HashSet<<Coin as HDWalletCoinOps>::Address>: From<HashSet<keys::Address>>,
    {
        ...
        let addresses: HashSet<_> = new_addresses
            .iter()
            .map(|address_balance| checked_address_from_str(coin, &address_balance.address).expect("Valid address"))
            .collect();
