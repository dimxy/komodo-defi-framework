...
pub async fn check_my_coin_balance_for_swap(
    ...
    let total_trade_fee = if ticker == trade_fee.coin {
        ...
        let platform_coin_balance: MmNumber = coin.platform_coin_balance().compat().await?.into();
        check_platform_coin_balance_for_swap(ctx, &platform_coin_balance, trade_fee, swap_uuid).await?;
        MmNumber::from(0)
    };
