...
async fn rpc_streaming_dispatcher(
    ...
    match streaming_request.as_str() {
        ...
        "fee_estimator::enable" => handle_mmrpc(ctx, request, streaming_activations::enable_fee_estimation_streaming).await,
        "fee_estimator::disable" => handle_mmrpc(ctx, request, streaming_activations::disable_fee_estimation_streaming).await,
