...
use mm2_test_helpers::for_tests::{
    ...
    wait_for_swap_finished, MarketMakerIt, Mm2TestConf, ARBITRUM_MAINNET_NODES, ARBITRUM_MAINNET_SWAP_CONTRACT,
...
fn test_aggregated_swap_mainnet_polygon_utxo_impl(
    ...
    let dai_order = block_on(create_maker_order(&mut mm_bob, DOC, &dai_ticker, 0.0011111, 2.0)).unwrap();
    let oneinch_order = block_on(create_maker_order(&mut mm_bob, DOC, &oneinch_ticker, 0.00105, 2.0)).unwrap();
    let agix_order = block_on(create_maker_order(&mut mm_bob, DOC, &agix_ticker, 0.000980009090, 2.0)).unwrap();
