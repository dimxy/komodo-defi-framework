...
fn legacy_swap_old_node_rejected(use_old_version_maker: bool) {
    let check_swap_failed = |mm: &MarketMakerIt, uuid| {
        let rc = block_on(mm.rpc(&json!({
            "userpass": mm.userpass,
            "method": "my_swap_status",
            "params": {
        ...
        let failed_event_type = events_array[1]["event"]["type"].as_str().unwrap();
        assert_eq!("NegotiateFailed", failed_event_type);
    };
    ...
    let coins = json!([mycoin_conf(1000), mycoin1_conf(1000)]);
    let mut envs = vec![];
    if use_old_version_maker {
        envs.push(("USE_OLD_VERSION_MAKER", "true"));
    } else {
        envs.push(("USE_OLD_VERSION_TAKER", "true"));
    }
    ...
    let wait_msg = format!("[swap uuid={}] Finished", sell_res.result.uuid);
    if use_old_version_maker {
        block_on(mm_alice.wait_for_log(22., |log| log.contains(&wait_msg))).unwrap();
    // old maker does not receive any messages if taker cancels negotiation, so we dont wait
    } else {
        block_on(mm_bob.wait_for_log(22., |log| log.contains(&wait_msg))).unwrap();
        // old taker does not receive any messages if maker cancels negotiation, so we dont wait
    }

    thread::sleep(Duration::from_secs(3));

    if use_old_version_maker {
        check_swap_failed(&mm_alice, &sell_res.result.uuid);
    } else {
        check_swap_failed(&mm_bob, &sell_res.result.uuid);
    }
    block_on(mm_bob.stop()).unwrap();
    block_on(mm_alice.stop()).unwrap();
}

// test swap negotiation must fail if maker version too old
#[test]
fn test_trade_legacy_swap_old_maker() { legacy_swap_old_node_rejected(true); }

// test swap negotiation must fail if taker version too old
#[test]
fn test_trade_legacy_swap_old_taker() { legacy_swap_old_node_rejected(false); }
